...
void decode_packet(unsigned char *buf, size_t length, FILE *lf) {
     ...
     unsigned short payload_len = ntohs(ip_head->tot_len) - ip_head_len;


     fprintf(lf, "\nIP payload length: %d bytes\n", payload_len);
...
void print_content(unsigned char *buf, size_t length, FILE *lf) {
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        if (buf[i] >= 32 && buf[i] <= 126) { 
               res_buf[res_index++] = (char)buf[i]; // Сохраняем печатаемый символ
               res_res_buf[res_res_index++] = (char)buf[i]; // Сохраняем печатаемый символ

        } else {
            res_buf[res_index++] = '.'; // Сохраняем точку для непечатаемых символов
        }

        if (i != 0 && i % 16 == 0) {
            fprintf(lf, "          ");
            for (size_t j = (i - 16); j < i; j++) {
                fprintf(lf, "%c", res_buf[j]); // Печатаем из res_buf
            }
            fprintf(lf, "\n");
        }
...
//     res_buf[res_index] = '\0'; // Завершаем строку в res_buf
     printf("Contents of res_res_buf: %s\n", res_res_buf);

     strcpy(buffer, res_res_buf); // Копируем содержимое res_buf в buffer
