...
# choose random mutation and random place for mutation
def mutations(a):
    # #for test
    # byte_idx = 15
    # # byte_idx = 19
    ...
    mutation_idx = random.randint(0, 8)

    print(f'Mutation of {mutation_idx} byte')
    if mutation_idx == 0:  # Bit flipping
        print('Mutation type:  Bit flipping')
        ...
        print('Mutation type:  Replace individual bytes with random values')
        a = [random.randint(0, 255) for _ in a]
        return bytes(a)
    elif mutation_idx == 5:  # Delete blocks of bytes
        print('Mutation type:  Delete blocks of bytes')
        block_size = random.randint(1, len(a) // 2)
        return bytes(a[block_size:])
    elif mutation_idx == 6:  # Replace blocks of bytes
        print('Mutation type:  Replace blocks of bytes')
        block_size = random.randint(1, len(a) // 2)
        return bytes([random.randint(0, 255) for _ in range(block_size)] + list(a)[block_size:])
    elif mutation_idx == 7:  # Add one to random byte
        print('Mutation type:  Add one to random byte')
        a = bytearray(a)
        byte_idx = random.randint(1, len(a)-1)
        a[byte_idx]=(a[byte_idx] + 1)%256

        print(f'{byte_idx=}, {a[byte_idx]=}')
        return bytes(a)
    elif mutation_idx == 8:
        print('Mutation type:  No mutation')
        return a
...
def layer_crossover(packet1, packet2):
    n = random.randint(1, min(len(packet2.layers()),len(packet1.layers()))-1)

    # print(packet1[n].payload,'\n',packet2[n])
    q = packet1.copy()
    q[n].payload = packet2[n]
...
def layer_crossover_mutation(packet1,packet2):
    ...
    data = layer_crossover(packet1,packet2)
